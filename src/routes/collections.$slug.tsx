...
import { collectionBySlug, templatesByCollection, KEYCHAIN_CATEGORIES, type TemplateCategory, type KeychainTemplate } from "@/lib/collections";
...
function CollectionPage() {
  ...
  return <KeychainGallery collectionTitle={collection.title} collectionDesc={collection.longDesc} templates={templatesByCollection(slug)} />;
}

function KeychainGallery({ collectionTitle, collectionDesc, templates }: { collectionTitle: string; collectionDesc: string; templates: KeychainTemplate[] }) {
  ...
  return (
    <Shell>
      <section className="mx-auto max-w-7xl px-5 pt-24 pb-8 sm:pt-32">
        <p className="text-[11px] font-semibold uppercase tracking-[0.28em] text-gold">Flagship collection</p>
        ...
        <p className="mt-5 max-w-xl text-base text-muted-foreground">{collectionDesc}</p>
      </section>

      <section className="sticky top-[68px] z-20 border-y border-border bg-background/85 backdrop-blur">
        <div className="mx-auto max-w-7xl px-5 py-4">
          <div className="flex flex-col gap-3 sm:flex-row sm:items-center">
            <div className="relative flex-1">
              ...
              <input
                ...
                className="h-11 w-full rounded-full border border-border bg-background pl-11 pr-4 text-sm outline-none transition focus:border-gold"
              />
            </div>
            <div className="text-xs text-muted-foreground sm:ml-3">{filtered.length} of {templates.length}</div>
          </div>
          <div className="mt-3 flex flex-wrap gap-2">
              ...
              return (
                <button
                  ...
                  className={[
                    ...
                    on ? "border-gold bg-gold text-white" : "border-border text-foreground/70 hover:border-foreground hover:text-foreground",
                  ].join(" ")}
                >
                  {cat}
                </button>
            ...
            {active.length > 0 && (
              <button onClick={() => setActive([])} className="ml-1 text-[12px] font-medium text-muted-foreground underline hover:text-foreground">Clear</button>
            )}
          </div>
        </div>
      </section>
      ...
      <section className="mx-auto max-w-7xl px-5 pb-28 pt-8">
        {filtered.length === 0 ? (
          ...
          <div className="grid grid-cols-2 gap-3 sm:gap-5 md:grid-cols-3 lg:grid-cols-4">
            {filtered.map((tpl) => (
              <TemplateCard key={tpl.id} t={tpl} fav={isFavorite(tpl.id)} onFav={() => toggleFavorite(tpl.id)} />
            ))}
          </div>
        )}
      </section>
...
export function TemplateCard({ t, fav, onFav }: { t: KeychainTemplate; fav: boolean; onFav: () => void }) {
  return (
    <article className="group flex flex-col">
      <div className="tls-zoom relative overflow-hidden rounded-2xl bg-secondary">
        ...
        <button
          ...
          className="absolute right-3 top-3 grid h-9 w-9 place-items-center rounded-full bg-white/90 text-foreground backdrop-blur transition hover:bg-white"
        >
      ...
      <div className="mt-3 flex items-start justify-between gap-2">
        <div className="min-w-0">
          <Link to="/template/$id" params={{ id: t.id }} className="block truncate font-display text-sm font-semibold tracking-tight hover:text-gold">{t.title}</Link>
          <p className="mt-0.5 text-[11px] uppercase tracking-[0.16em] text-muted-foreground">{t.category}{t.difficulty ? ` · ${t.difficulty}` : ""}</p>
        </div>
        <Link to="/template/$id/customize" params={{ id: t.id }} className="shrink-0 rounded-full bg-foreground px-3 py-1.5 text-[11px] font-semibold text-background transition hover:bg-gold">Customize</Link>
      </div>
    </article>
  );
}