...
export function LinePage({ lineKey }: { lineKey: LineKey }) {
  ...
  const isNoir = lineKey === "metal";
  return (
    <Shell>
      <div className={isNoir ? "noir" : ""}>
        <section className="mx-auto max-w-7xl px-5 pt-12 sm:pt-20">
          ...
          <h1 className="mt-3 text-5xl font-black tracking-tight sm:text-7xl">
            <span className="gold-text">{t(line.title, line.titleKm, lang)}</span>
          </h1>
          <p className="mt-4 max-w-xl text-base text-muted-foreground sm:text-lg">{line.subtitle}</p>
        </section>
        <section className="mx-auto mt-12 grid max-w-7xl gap-x-6 gap-y-10 px-5 pb-20 sm:grid-cols-2 lg:grid-cols-3">
          {products.map((p) => <ProductCard key={p.id} p={p} noir={isNoir} />)}
        </section>
      </div>
    </Shell>
  );
